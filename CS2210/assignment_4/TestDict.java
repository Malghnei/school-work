	...
	public static void main(String[] args) {
		...
		if (alltests || test == 1 || test == 3 || test == 6 || test == 7 || test == 8 || test == 9)
			try {
				...
				if (!(list.get(0).getContent()).equals(content[0]))
					passed = false;
				BSTNode r = dictionary.getRoot();
				if (!r.getData().getName().equals(keys[0]))
					passed = false;

				list = dictionary.get(root, keys[1]);
				if (list != null)
					passed = false;
				if (dictionary.getNumInternalNodes() != 1)
					passed = false;
				print(passed, 1);
			} catch (Exception e) {
				print(false, 1);
		...
		if (alltests || test == 5)
			try {
				...
				if (!root.getLeftChild().getData().getName().equals("computer"))
					passed = false;
				dictionary.remove(root, keys[1]);
				String k = root.getData().getName();
				if (!k.equals("computer") && !k.equals("four"))
					passed = false;
				dictionary.remove(root, keys[2]);
				print(false, 4);
			} catch (DictionaryException e) {
		...
		if (alltests || test == 6 || test == 7 || test == 8 || test == 9)
			try {
				...
				if ((data.getName()).compareTo(keys[1]) == 0)
					if (dictionary.getNumInternalNodes() != 5)
						passed = false;
		...
		if (alltests || test == 7)
			try {
				...
				if ((data.getName()).compareTo(keys[3]) != 0)
					passed = false;

				data = dictionary.smallest(root);
				if ((data.getName()).compareTo(keys[2]) != 0)
					passed = false;
				print(passed, 6);
			} catch (Exception e) {
				print(false, 6);
		...
		if (alltests || test == 8)
			try {
				...
				if ((data.getName()).compareTo(keys[4]) != 0)
					passed = false;
				data = dictionary.largest(root);
				if ((data.getName()).compareTo(keys[0]) != 0)
					passed = false;
		...
		try {
			...
			BufferedReader in = new BufferedReader(new FileReader("filesProvided/large.txt"));
			...
			if (alltests || test == 11)
				...
				try {
					...
					if (list != null)
						passed = false;
					data = dictionary.smallest(root);
					if (data.getName().compareTo("aaronical") != 0)
						passed = false;
			...
			if (alltests || test == 13)
				...
				try {
					passed = true;
					data = dictionary.largest(root);
					if (data.getName().compareTo("zythum") != 0)
						passed = false;
